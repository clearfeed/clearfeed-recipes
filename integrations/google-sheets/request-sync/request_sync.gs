// Clearfeed to Google Sheets Sync Script
  ...
  const CONFIG = {
    API_KEY: "PAT_USER_TOKEN", // Replace with your Clearfeed API key
      ...
      try {
        ...
        switch (header) {
          ...
          case 'First Response Time (mins)':
            if (request.sla_metrics && request.sla_metrics.first_response_time && request.sla_metrics.first_response_time.value !== null &&
  request.sla_metrics.first_response_time.value !== undefined) {
              return String(request.sla_metrics.first_response_time.value);
            }
            return '';

          case 'First Response Time Breached':
            if (request.sla_metrics && request.sla_metrics.first_response_time && typeof request.sla_metrics.first_response_time.is_breached
  === 'boolean') {
          ...
          case 'Resolution Time (mins)':
            if (request.sla_metrics && request.sla_metrics.resolution_time && request.sla_metrics.resolution_time.value !== null &&
  request.sla_metrics.resolution_time.value !== undefined) {
              return String(request.sla_metrics.resolution_time.value);
            }
            return '';

          case 'Resolution Time Breached':
            if (request.sla_metrics && request.sla_metrics.resolution_time && typeof request.sla_metrics.resolution_time.is_breached ===
  'boolean') {
          ...
          case 'First Resolution Time (mins)':
            if (request.sla_metrics && request.sla_metrics.first_resolution_time && request.sla_metrics.first_resolution_time.value !== null &&
   request.sla_metrics.first_resolution_time.value !== undefined) {
  ...
  function enableHourlyTrigger() {
    ...
    SpreadsheetApp.getUi().alert('Success', 'Hourly automatic sync has been enabled. The script will now run every hour.',
  ...
  function disableHourlyTrigger() {
    ...
    if (deletedCount > 0) {
      ...
      SpreadsheetApp.getUi().alert('Info', 'No hourly triggers were found. Automatic sync was already disabled.',
  SpreadsheetApp.getUi().ButtonSet.OK);
    }
  }
